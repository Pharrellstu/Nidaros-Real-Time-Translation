...
public class WhisperSpeechToTextHandler implements SpeechHandler
{
    ...
    private static final Counter CAPTION_COUNTER = Counter.builder("rttranslator_captions_total")
            ...
            .tag("engine", ENGINE_VALUE)
            .register(MetricsRegistry.registry());
    private static final ConcurrentMap<String, Counter> LANGUAGE_CAPTION_COUNTERS = new ConcurrentHashMap<>();
    private static final ConcurrentMap<String, Timer> LANGUAGE_LATENCY_TIMERS = new ConcurrentHashMap<>();
            ...
            for (Map.Entry<String, LinkedList<CaptionLine>> entry : captionLines.entrySet())
            {
                ...
                synchronized (lines)
                {
                    ...
                    if (!textList.isEmpty())
                    {
                        ...
                        recordLatency(language, earliestLineTimestamp);
                        captionCounterForLanguage(language).increment();
                    }
                }
            }
    ...
    private void recordLatency(String language, long earliestLineTimestamp)
    {
        ...
        latencyTimerForLanguage(language).record(latencyMs, TimeUnit.MILLISECONDS);
    }

    private Counter captionCounterForLanguage(String language)
    {
        String normalized = normalizeLanguage(language);
        return LANGUAGE_CAPTION_COUNTERS.computeIfAbsent(normalized, lang ->
                Counter.builder("rttranslator_translation_captions_total")
                        .description("Captions emitted per language")
                        .tag("engine", ENGINE_VALUE)
                        .tag("language", lang)
                        .register(MetricsRegistry.registry()));
    }

    private Timer latencyTimerForLanguage(String language)
    {
        String normalized = normalizeLanguage(language);
        return LANGUAGE_LATENCY_TIMERS.computeIfAbsent(normalized, lang ->
                Timer.builder("rttranslator_translation_latency_seconds")
                        .description("Latency per language from Whisper response to caption dispatch")
                        .tag("engine", ENGINE_VALUE)
                        .tag("language", lang)
                        .publishPercentileHistogram()
                        .register(MetricsRegistry.registry()));
    ...
    {
        return (language == null || language.isBlank()) ? "unknown" : language;
    }
